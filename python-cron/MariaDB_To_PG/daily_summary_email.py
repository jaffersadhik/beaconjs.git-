...
def create_excel(rpt_mode, rpt_dt, pg_conn, cfg_val):
    ...
    if rpt_mode == "date":
        ...
        df_oper['invoice_based_on'] = df_oper['invoice_based_on'].\
                                    apply(lambda v: 'Submission' if v == 0 else 'Deliveries')
...
def main():
    ...
    if rpt_mode == "date":
        if 'RPT_DATE' in cfg_val:
            cfg_rpt_dt = cfg_val['RPT_DATE'].upper()
            if cfg_rpt_dt == "DEFAULT":
                rpt_dt = dt_cur - relativedelta(days=1)
            else:
                rpt_dt = datetime.strptime(cfg_rpt_dt, "%Y-%m-%d")
        else:
            rpt_dt = dt_cur - relativedelta(days=1)
