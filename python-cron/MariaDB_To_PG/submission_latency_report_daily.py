...
def gen_lat_rpt(cfg_val, rpt_dt, pg_conn):
    ...
    xls_fn_lst = []
    lat_sql_count = """with vw_lat as (
select
	recv_date,
	sub_carrier,
	count(*) total_count,
	SUM(case when del_delv_lat_ori_in_millis <= 100 then 1 else 0 end) as LTE_100MS,
	SUM(case when del_delv_lat_ori_in_millis >101 and del_delv_lat_ori_in_millis <= 200 then 1 else 0 end) as LTE_200MS,
	SUM(case when del_delv_lat_ori_in_millis >201 and del_delv_lat_ori_in_millis <= 300 then 1 else 0 end) as LTE_300MS,
	SUM(case when del_delv_lat_ori_in_millis >301 and del_delv_lat_ori_in_millis <= 400 then 1 else 0 end) as LTE_400MS,
	SUM(case when del_delv_lat_ori_in_millis >401 and del_delv_lat_ori_in_millis <= 500 then 1 else 0 end) as LTE_500MS,
	SUM(case when del_delv_lat_ori_in_millis >501 and del_delv_lat_ori_in_millis <= 700 then 1 else 0 end) as LTE_700MS,
	SUM(case when del_delv_lat_ori_in_millis >701 and del_delv_lat_ori_in_millis <= 1000 then 1 else 0 end) as LTE_1000MS,
	SUM(case when del_delv_lat_ori_in_millis >1001 and del_delv_lat_ori_in_millis <= 1200 then 1 else 0 end) as LTE_1200MS,
	SUM(case when del_delv_lat_ori_in_millis >1201 and del_delv_lat_ori_in_millis <= 1500 then 1 else 0 end) as LTE_1500MS,
	SUM(case when del_delv_lat_ori_in_millis >1501 and del_delv_lat_ori_in_millis <= 2000 then 1 else 0 end) as LTE_2000MS,
	SUM(case when del_delv_lat_ori_in_millis > 2000 then 1 else 0 end) as GT_2000MS
from
	smslog.sub_del_log_{}
where
    sub_sub_ori_sts_code = '400'
    and sub_intf_grp_type = 'api'
group by
	recv_date,
	sub_carrier )
select
...
order by
    ...
    xls_fn = "./xls_daily/Submission_Latency_Count_Report_" + rpt_dt.strftime("%d_%b_%Y") + ".xlsx"
    ...
    xls_fn = "./xls_daily/Submission_Latency_Percentage_Report_" + rpt_dt.strftime("%d_%b_%Y") + ".xlsx"
...
def main():
    ...
    log_fn = "./log/submission_latency_report_email_" + log_fn_dtsfx + ".log"
    logging.basicConfig(filename=log_fn, level=logging.INFO,
                        format='%(asctime)s %(levelname)-8s %(funcName)-20s %(message)s')
    logging.info("Submission Latency Report Email started")
    ...
    pg_conn.close()
    logging.info("Submission Latency Report Email completed")
