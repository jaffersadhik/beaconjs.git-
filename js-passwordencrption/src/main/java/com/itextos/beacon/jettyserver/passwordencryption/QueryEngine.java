package com.itextos.beacon.jettyserver.passwordencryption;
...
public class QueryEngine
{
	...
	public static void addPassword(Server server2) {
		
		final ServletContextHandler handler = new ServletContextHandler(server2, "/password");

        log.info("Preparing api routes");
        handler.addServlet(com.itextos.beacon.commonlib.encryption.servlet.EncryptionServlet.class.getName(), "/encryption");
        
        handler.addServlet(com.itextos.beacon.commonlib.password.servlet.PasswordResetServlet.class.getName(), "/reset");
