...
async function downloadlog(fastify, opts) {
    ...
    fastify.post('/downloadlog', { schema: downloadlogSchema }, async (req, reply) => {
        try {
            ...
            const { cli_id, tz, user, user_type } = req.user;
            const { LOG_DOWNLOAD_URL } = process.env;
            const respArr = [];

            console.log('/downloadlog incoming params => ', [dateselectiontype, fdate, tdate, source, campaign_id, campaign_name, senderid, status]);

            const { fromdatetimeStrInIST, todatetimeStrInIST } = await fastify.getFromAndToDate(tz, dateselectiontype, fdate, tdate, true);
            console.log('/downloadlog from and to date => ', fromdatetimeStrInIST, todatetimeStrInIST);
            ...
            console.log(`/downloadlog is full msg enabled for cli_id ${cli_id}... ${isFullMsgEnabled ? 'YES' : 'NO'}`);
            ...
            console.log('Resp from api => ', r.data);
