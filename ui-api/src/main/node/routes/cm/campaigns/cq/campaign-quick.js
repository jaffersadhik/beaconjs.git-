...
const { cqSendSchema, cqScheduleSchema } = require('../../../../schema/campaigns-schema');

async function campaignsQuick(fastify, opts) {
  ...
  async function processCampaign(req) {
    const { c_name, mobile_list, remove_dupe_yn, msg, c_type, c_lang_type, c_lang, valid, dlt_entity_id, header, shorten_url_yn, intl_header, traffic_to } = req.body;
    let { template_id, dlt_template_id } = req.body;
    console.log('processCampaign() incoming params => ', [c_name, mobile_list, remove_dupe_yn, msg, c_type, c_lang_type, c_lang, valid, dlt_entity_id, header, shorten_url_yn, dlt_template_id, intl_header, traffic_to]);
    ...
    // write mobile list to a file
    // const basePath = '/Volumes/SP1/test';
    const basePath = _.get(result, 'value');
    const filename = `cq_${fastify.nanoid()}_${moment().format('YYYYMMDD')}.csv`;
    const fileloc = path.join(basePath, _.toLower(user), filename);

    fs.mkdirSync(path.dirname(fileloc), { recursive: true });
    fs.writeFileSync(fileloc, mobile_list.join(os.EOL));

    // prepare the obj for db insert
    const rFilename = filename;
    const ext = path.extname(rFilename);
    // id, c_id, filename, filename_ori, filetype, fileloc, total
    valuesArr.push([fastify.nanoid22(), id, rFilename, filename, ext, fileloc, valid]);
  ...
  fastify.post('/send', { schema: cqSendSchema }, async (req, reply) => {
    try {
      ...
      if (+bill_type === 1) {
        ...
        console.log('/send resp from redis for user => ', cli_id, amount);
  ...
  fastify.post('/schedule', { schema: cqScheduleSchema }, async (req, reply) => {
    try {
      // TODO: Check campaign name uniqueness
