...
async function auth(fastify, opts) {
    ...
    fastify.get('/dprocessedstats', { preValidation: [], schema: dprocessedstatsSchema }, async (req, reply) => {
        try {
            ...
            if (_.eq(dateselectiontype, 'this month')) {
                ...
                const [rThismonth] = await Promise.all([fastify.getRSOverall(thismonthfd, thismonthtd, interfaceArr, 'all', 'all', cli_id)]);
    ...
    fastify.get('/dtrend', { preValidation: [], schema: dtrendSchema }, async (req, reply) => {
        try {
            ...
            if (_.eq(dateselectiontype, 'this month')) {
                console.log('/dtrend [this month] from and to date => ', [todayfts, todaytts], [thismonthfd, thismonthtd]);
                const [rTodayMTSuccess, rTodayDNSuccess, rThismonth] = await Promise.all([fastify.getTodaysCountByStatus(cli_id, todayfts, todaytts, 'mtsuccess'), fastify.getTodaysCountByStatus(cli_id, todayfts, todaytts, 'dnsuccess'), fastify.getRSDatewise(thismonthfd, thismonthtd, interfaceArr, 'all', 'all', cli_id)]);
