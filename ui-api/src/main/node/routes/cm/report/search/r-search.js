...
async function rsearch(fastify, opts) {
    ...
    async function processSearch(req) {
        const { input, search_for, dateselectiontype, fdate, tdate } = req.body;
        const { tz, cli_id } = req.user;
        const { LOG_DATA_URL } = process.env;

        console.log('/rlogsearch incoming params => ', [input, search_for, dateselectiontype, fdate, tdate]);
    ...
    fastify.post('/rlogsearchdownload', { preValidation: [], schema: rlogsearchSchema }, async (req, reply) => {
        try {
            ...
            const header = 'Accepted Date,Submitted Date,Delivered Date,Sender Id,Mobile,Status,Message,SMS Rate (INR), DLT Rate (INR), ACK ID, Reason';
            ...
            for (const o of payload) {
                arr.push(`${o.recv_time},${o.sub_carrier_sub_time},${o.del_dly_time},${o.sub_cli_hdr},${o.dest},${o.status},"${_.replace(o.sub_msg, /"/g, '""')}",${o.sms_rate},${o.dlt_rate},${o.sub_file_id},${o.reason}`);
            }
