...
async function report(fastify, opts) {
    ...
    fastify.post('/rsources', { preValidation: [], schema: rSourceSchema }, async (req, reply) => {
        try {
            ...
            if (_.eq(typeofdate, 'onlytoday')) {
                ...
                const r1 = await fastify.getRSourcesForToday(cli_id, fromdatetimeStrInIST, todatetimeStrInIST);
                ...
                const result = await fastify.getRSources(cli_id, fromdateStrInIST, todateStrInIST);
                console.log('/rsources resp from db => ', result);
                payload.push(...result);
            } else {
                // both
                console.log('/rsources [both] getting data...');
                const [r1, r2] = await Promise.all([fastify.getRSourcesForToday(cli_id, fromdatetimeStrInIST, todatetimeStrInIST), fastify.getRSources(cli_id, fromdateStrInIST, todateStrInIST)]);
                const b1 = _.get(r1, 'aggregations.sources.buckets', []);
                console.log('/rsources resp from es =>', b1);
                const arr = [];
                for (const v of b1) {
    ...
    fastify.post('/rcampaigns', { preValidation: [], schema: rCampaignsSchema }, async (req, reply) => {
        try {
            const { dateselectiontype, fdate, tdate, source } = req.body;
            const { tz, cli_id } = req.user;
            ...
            const r1 = await fastify.getRCampaigns(cli_id, fromdatetimeStrInIST, todatetimeStrInIST);
    ...
    fastify.post('/rsenderids', { preValidation: [], schema: rSenderIdsSchema }, async (req, reply) => {
        try {
            const { dateselectiontype, fdate, tdate, source, campaign_id } = req.body;
            const { tz, cli_id } = req.user;
            ...
            if (_.eq(typeofdate, 'onlytoday')) {
                ...
                const r1 = await fastify.getRSenderIdsForToday(cli_id, fromdatetimeStrInIST, todatetimeStrInIST, source, campaign_id);
                ...
                const result = await fastify.getRSenderIds(cli_id, fromdateStrInIST, todateStrInIST, interfaceArr, campaign_id);
                console.log('/rsenderids resp from db => ', result);
                senderidArr.push(..._.map(result, 'cli_hdr'));
            } else {
                // both
                console.log('/rsenderids [both] getting data...');
                const [r1, r2] = await Promise.all([fastify.getRSenderIdsForToday(cli_id, fromdatetimeStrInIST, todatetimeStrInIST, source, campaign_id), fastify.getRSenderIds(cli_id, fromdateStrInIST, todateStrInIST, interfaceArr, campaign_id)]);
                const b1 = _.get(r1, 'aggregations.senderid.buckets', []);
                console.log('/rsenderids resp from es =>', b1);
                ...
                console.log('/rsenderids resp from db => ', r2);
