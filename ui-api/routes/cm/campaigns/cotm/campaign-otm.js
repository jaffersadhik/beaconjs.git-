...
const { cotmSendSchema, cotmScheduleSchema } = require('../../../../schema/campaigns-schema');

async function campaignsOTM(fastify, opts) {
  ...
  async function processCampaign(req) {
    const { c_name, remove_dupe_yn, msg, c_type, c_lang_type, c_lang, dlt_entity_id, header, files, shorten_url_yn, intl_header, traffic_to } = req.body;
  ...
  fastify.post('/send', { schema: cotmSendSchema }, async (req, reply) => {
    try {
  ...
  fastify.post('/schedule', { schema: cotmScheduleSchema }, async (req, reply) => {
    try {
      const obj = await processCampaign(req);
